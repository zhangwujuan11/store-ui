<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="类别" prop="paramType">
        <el-select v-model="queryParams.paramType" placeholder="请选择类别" clearable size="small">
    ...
    <el-table v-loading="loading" :data="typeDataList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="类别" align="center" prop="paramType" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="类别主键id" prop="typeId">
          <el-input v-model="form.typeId" placeholder="请输入类别主键id" />
        </el-form-item>
        <el-form-item label="类别" prop="paramType">
          <el-select v-model="form.paramType" placeholder="请选择类别">
            <el-option label="请选择字典生成" value="" />
          </el-select>
        </el-form-item>
        <el-form-item label="编码" prop="paramCode">
          <el-input v-model="form.paramCode" placeholder="请输入编码" />
        </el-form-item>
        <el-form-item label="名称" prop="paramName">
...
<script>
import { listTypeData, getTypeData, delTypeData, addTypeData, updateTypeData, exportTypeData } from "@/api/store/typeData";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        typeId: [
          { required: true, message: "类别主键id不能为空", trigger: "blur" }
        ],
        paramType: [
  ...
  methods: {
    ...
    getList() {
      ...
      listTypeData(this.queryParams).then(response => {
        this.typeDataList = response.rows;
